...
abstract class Union2<A, B> {
  const factory Union2.first(A value) = _Union2First;
  const factory Union2.second(B value) = _Union2Second;

  R join<R>(R first(A value), R second(B value));
}

class _Union2First<A, B> implements Union2<A, B> {
  const _Union2First(this.a);

  final A a;

  @override
  R join<R>(R Function(A value) first, R Function(B value) second) => first(a);

  @override
  int get hashCode => a.hashCode;

  @override
  bool operator ==(Object other) => other is _Union2First<A, B> && other.a == a;
}

class _Union2Second<A, B> implements Union2<A, B> {
  const _Union2Second(this.a);

  final B a;

  @override
  R join<R>(R Function(A value) first, R Function(B value) second) => second(a);
